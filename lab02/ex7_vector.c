...
vector_t *vector_new() {
    ...
    retval = malloc(sizeof(struct vector_t));/* YOUR CODE HERE */
...
void vector_set(vector_t *v, size_t loc, int value) {
    ...
    if(loc >= v->size){
        int *tmp = malloc(sizeof(int)*(loc+1));
        if(tmp == NULL){
            allocation_failed();
        }
        memcpy(tmp, v->data, sizeof(int)*v->size);
        memset(tmp+v->size, 0, sizeof(int)*(loc+1-v->size));
        free(v->data);
        v->data = tmp;
        v->size = loc + 1;
    }
